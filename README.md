...
- Trying to fix vector size with chibicc seems to have a bug with typedef that doesn't retrieve the correct alignment. Not sure yet what is the root cause. The scope or type returned by hashmap_get2 seems to be wrong. The second issue with vector size is that it causes a segmentation fault or returns wrong values due probably to misalignment somewhere in assembly code.
