...
temp stack from widcc/slimcc (@fuhsnn) but still an issue with some structs in variadic with alignment > 16. Fixing issue with bitfield (using fixes from widcc @fuhsnn). Integrating fix from @fuhsnn/widcc about VLA typedef and commit 42b33ec. Fixing issue with align > 16 during variadic functions(workaround in stdarg.h). Fixing ISS-188 on old C style (K&R) parameter omitted by mistake.
