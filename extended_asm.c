...
static void mark_asm_var_liveness(Token *tok, bool is_write, bool is_read) {
  VarScope *sc = find_var(tok);
  if (sc && sc->var && !sc->var->is_function) {
    if (is_read) sc->var->is_read = true;
    if (is_write) sc->var->is_written = true;
  }
}
...
static void ensure_input_reg(AsmInput *in, char *preferred64) {
    ...
    if (!in->reg)
        error("%s:%d: in %s: reg is null!", __FILE__, __LINE__, __func__);
    in->reg64 = in->reg;
    ...
    while (!equal(tok->next, ";") && !equal(tok, ";"))
    {
        ...
        case AT_OUTPUT: // outputs
            ...
            if (asmExt->output[nbOutput]->variableNumber) {
                ...
                if (!output_loading)
                 error("%s:%d: in %s: output_loading is null!", __FILE__, __LINE__, __func__);
                if (!asmExt->output[nbOutput]->reg)
                error("%s:%d: in %s: asmExt->output[nbOutput]->reg is null!", __FILE__, __LINE__, __func__);
        ...
        case AT_INPUT: // inputs
            ...
            if (asmExt->input[nbInput]->variableNumber) {
                ...
                if (!input_asm_str)
                    error("%s:%d: in %s: input_asm_str is null!", __FILE__, __LINE__, __func__);
                if (!asmExt->input[nbInput]->reg)
                    error("%s:%d: in %s: asmExt->input[nbInput]->reg is null!", __FILE__, __LINE__, __func__);
        ...
        case AT_LABEL: // labels
            ...
            *rest = tok;            
            break;
        default: // error
            error_tok(tok, "%s:%d: in %s: too much parameters or complex extended assembly not managed!", __FILE__, __LINE__, __func__);
        }
        if (equal(tok, ":")) {
            asmtype += 1;
            tok = tok->next;
        ...
        for (int i = 0; i < nbOutput; i++)
        {
            ...
            if (asmExt->output[i]->isAddress) {
                ...
                if (!asmExt->output[i]->variableNumber)
                  error("%s:%d: in %s: variableNumber is null! template=%s i=%d", __FILE__, __LINE__, __func__, template, i);
...
static bool parse_simple_binary_input(Token **rest, Token *tok, Obj *locals)
{
    ...
    if (lhs->kind == TK_IDENT) {
        ...
        if (sc->enum_ty) {
            ...
            mark_asm_var_liveness(lhs, false, true);
    ...
    if (rhs->kind == TK_IDENT) {
        ...
        if (sc->enum_ty) {
            ...
            mark_asm_var_liveness(rhs, false, true);
    ...
    in->size = size;
    if (!in->reg)
        error_tok(lhs, "%s:%d: in %s: reg is null!", __FILE__, __LINE__, __func__);
    ...
    while (!equal(tok->next, ":") && !equal(tok->next, ";"))
    {
        ...
        if (equal(tok, "[") || equal(tok, "{"))
        {
            ...
            if (tok->kind == TK_IDENT ) {
                ...
                if (!equal(tok, (char[]){closing, 0})) {
                    error_tok(tok, "%s:%d: in %s: expected closing %c after operand name", __FILE__, __LINE__, __func__, closing);
                }
                tok = tok->next;
            }
        ...
        else if (equal(tok, "("))
        {
            ...
            if (tok->kind == TK_IDENT)
            {
                ...
                if (!asmExt->output[nbOutput]->reg)
                    error_tok(tok, "%s:%d: in %s: reg is null extended assembly not managed yet", __FILE__, __LINE__, __func__);
...
// Handle struct member outputs like `r.a` or `ptr->a`.
                ...
                if ((sc->var->ty->kind == TY_STRUCT || sc->var->ty->kind == TY_UNION) && equal(member_tok, ".")) {
                    ...
                    if (tokmbr->kind != TK_IDENT)
                        error_tok(tokmbr, "%s:%d: in %s: expected member identifier", __FILE__, __LINE__, __func__);

                    Member *mbr = find_struct_member(base, tokmbr);
                    if (!mbr)
                        error_tok(tokmbr, "%s:%d: in %s: no such member", __FILE__, __LINE__, __func__);
                ...
                if (sc->var->ty->kind == TY_PTR && equal(member_tok, "->")) {
                    ...
                    if (!base)
                        error_tok(tok, "%s:%d: in %s: expecting struct base but base is null!", __FILE__, __LINE__, __func__);
                    if (!base->members)
                        error_tok(tok, "%s:%d: in %s: expecting members but members is null", __FILE__, __LINE__, __func__);
                ...
                tok = tok->next;                
                if (!equal(tok, ")"))
                    error_tok(tok, "%s:%d: in %s: extended assembly not managed yet", __FILE__, __LINE__, __func__);
                SET_CTX(ctx);
                while (equal(tok, ")")) tok = tok->next;
                    *rest = tok;
                return;
                ...
                if (tok->kind == TK_IDENT)
                {
                    ...
                    *rest = tok;
                    return;
                            ...
                            tok = tok->next->next;
                            if (!equal(tok, "void"))
                                error_tok(tok, "%s:%d: in %s: expected cast to (void **)", __FILE__, __LINE__, __func__);
                            ...
                            if (!equal(tok, ")"))
                                error_tok(tok, "%s:%d: in %s: expected ')' after cast type", __FILE__, __LINE__, __func__);

                            tok = tok->next; 

                            if (tok->kind != TK_IDENT)
                                error_tok(tok, "%s:%d: in %s: expected identifier after cast", __FILE__, __LINE__, __func__);
...
void input_asm(Node *node, Token **rest, Token *tok, Obj *locals)
{
    ...
    while (!equal(tok->next, ":") && !equal(tok->next, ";"))
    {
        ...
        else if (tok->kind == TK_STR && !strncmp(tok->str, "q", tok->len))
        {
            ...
            if (!asmExt->input[nbInput]->reg)
                 error_tok(tok, "%s:%d: in %s: reg is null!", __FILE__, __LINE__, __func__);            
            asmExt->input[nbInput]->reg64 = asmExt->input[nbInput]->reg;
            asmExt->input[nbInput]->regh = register_higher(asmExt->input[nbInput]->reg64);
            asmExt->input[nbInput]->regl = register_lower(asmExt->input[nbInput]->reg64);
            asmExt->input[nbInput]->regw = register_word(asmExt->input[nbInput]->reg64);            
        ...
        else if (equal(tok, "("))
        {
            ...
            {           
                ...
                if (!sc->var->ty)
                    error_tok(tok, "%s:%d: in %s: variable type unknown", __FILE__, __LINE__, __func__);
                mark_asm_var_liveness(tok, false, true);
                asmExt->input[nbInput]->input = tok;
                ...
                if (!asmExt->input[nbInput]->reg) {
                    error_tok(tok, "%s:%d: in %s: reg is null! %d", __FILE__, __LINE__, __func__, nbInput);
                }
                    ...
                    return;
                       // Handle casted input like *((void **)ptr)
                        ...
                        if (!sc || !sc->var)
                            error_tok(tok, "%s:%d: in %s: variable undefined", __FILE__, __LINE__, __func__);
                        if (!sc->var->ty)
                            error_tok(tok, "%s:%d: in %s: variable type undefined", __FILE__, __LINE__, __func__);
                        mark_asm_var_liveness(tok, false, true);
                        ...
                        tok = tok->next;
                        if (!equal(tok, ")"))
                            error_tok(tok, "%s:%d: in %s: expected ')' after cast", __FILE__, __LINE__, __func__);
                        tok = tok->next; 
            ...
            error_tok(tok, "%s:%d: in %s: input complex constraint not managed yet! %.*s", __FILE__, __LINE__, __func__, tok->len, tok->loc);
        }
...
{
    if (!oldstr)
        error("%s:%d: in %s: oldstr is null ", __FILE__, __LINE__, __func__);
    if (!newstr )
        return oldstr;
    if (!str)
        error("%s:%d: in %s: str is null ", __FILE__, __LINE__, __func__);
    size_t cap = 10000;
    char bstr[10000];
    ...
    for (i = 0; i < strlen(str); i++)
    {
        ...
        if (strlen(bstr) + 2 >= cap)
            error("%s:%d: in %s: not enough memory!", __FILE__, __LINE__, __func__);
    }
...
char *generate_input_asm(char *input_str)
{
    ...
    error("%s:%d: in %s: unexpected error! %s", __FILE__, __LINE__, __func__, asmExt->template->templatestr);
    //return NULL;
}
...
char *load_variable(int offset)
{
    ...
    if (length < 0)
        error("%s:%d: in %s: error during snprintf function! offset=%d length=%d", __FILE__, __LINE__, __func__, offset, length);
...
char *opcode(int size)
{
    ...
    case 16:
        return "  mov ";        
    default:
        error("%s:%d: in %s: size %d unexpected!", __FILE__, __LINE__, __func__, size);
    }
}
...
char * retrieveVariableNumber(int index) 
{
    ...
    if (index > 8)
        error("%s:%d: in %s: index %d not managed yet!", __FILE__, __LINE__, __func__, index);
    char *tmp;        
    ...
    if (length < 0)
        error("%s:%d: in %s: error during snprintf function! index=%d length=%d", __FILE__, __LINE__, __func__, index, length);
