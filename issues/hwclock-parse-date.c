...
{
	if (pc->dates_seen && !pc->year.digits && !pc->rels_seen && (pc->times_seen || 2 < text_int.digits))
	{
		pc->year = text_int;
	}
	else
...
#line 406 "sys-utils/hwclock-parse-date.y"
	{
		pc->zones_seen++;
		if (!time_zone_hhmm(pc, (yyvsp[-1].textintval), (yyvsp[0].textintval)))
			YYABORT;
	}
...
static int to_hour(intmax_t hours, int meridian)
{
	...
	case MERam:
		return 0 < hours && hours < 12 ? hours : hours == 12 ? 0
															 : -1;
	case MERpm:
		return 0 < hours && hours < 12 ? hours + 12 : hours == 12 ? 12
																  : -1;
	}
}
			   ...
			   struct timespec const *now)
{
	 ...
	 */
	{
		...
		for (quarter = 1; quarter <= 3; quarter++)
		{
			...
			if (probe_tm && probe_tm->tm_zone && probe_tm->tm_isdst != pc.local_time_zone_table[0].value)
			{
				{
	...
	if (pc.local_time_zone_table[0].name && pc.local_time_zone_table[1].name && !strcmp(pc.local_time_zone_table[0].name, pc.local_time_zone_table[1].name))
	{
		/**
	...
	{
		if (1 < (pc.times_seen | pc.dates_seen | pc.days_seen | pc.dsts_seen | (pc.local_zones_seen + pc.zones_seen)))
		{
			goto fail;
		}
		...
		if (!mktime_ok(&tm0, &tm, Start))
		{
			...
			else
			{
				...
				intmax_t abs_time_zone_hour = abs_time_zone / 60;

				int abs_time_zone_min = abs_time_zone % 60;

				char tz1buf[sizeof "XXX+0:00" + sizeof pc.time_zone * CHAR_BIT / 3];
		...
		{
			tm.tm_mday += ((pc.day_number - tm.tm_wday + 7) % 7 + 7 * (pc.day_ordinal - (0 < pc.day_ordinal && tm.tm_wday != pc.day_number)));
			tm.tm_isdst = -1;
			Start = mktime(&tm);
			if (Start == (time_t)-1)
			{
		...
		if (pc.rel.year | pc.rel.month | pc.rel.day)
		{
			...
			if (((year < tm.tm_year) ^ (pc.rel.year < 0)) | ((month < tm.tm_mon) ^ (pc.rel.month < 0)) | ((day < tm.tm_mday) ^ (pc.rel.day < 0)))
			{
				goto fail;
			}
			tm.tm_year = year;
		...
		if ((d1 / (60 * 60) ^ pc.rel.hour) | (d2 / 60 ^ pc.rel.minutes) | ((t1 < t0) ^ (d1 < 0)) | ((t2 < t1) ^ (d2 < 0)) | ((t3 < t2) ^ (d3 < 0)) | ((t4 < t3) ^ (d4 < 0)) | (t5 != t4))
		{
			goto fail;
		}
		result->tv_sec = t5;
